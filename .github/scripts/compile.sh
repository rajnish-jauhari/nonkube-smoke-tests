...
sudo python3 -m pip install --upgrade -r "${SKUPPER_DIR}"/requirements-dev.txt
sudo python3 -m pip install --upgrade -r "${PROTON_DIR}"/python/ci_requirements.txt
...
sudo python3 -m pip install "$(find "$PROTON_BUILD_DIR/python/dist" -name 'python_qpid_proton*.whl')"
...
sudo python3 -m pip install --disable-pip-version-check --ignore-installed --prefix="$PROTON_INSTALL_DIR/usr" "$(find "$PROTON_BUILD_DIR/python/dist" -name 'python_qpid_proton*.whl')"
